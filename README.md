...
The Maven release line starts at **1.0.0** under `me.alphatct3209`; the current release is **1.6.7**. Versioning follows the project policy: feature releases increment the middle number, fixes/minor updates increment the patch number, and exceptionally large compatibility generations increment the first number.
...
Updates do not require deleting the HyXDuels data folder. On startup, `config.yml` receives newly shipped defaults and every bundled `advanced/*.yml` file is synchronized against its versioned schema: missing files and keys are added, administrator values and unknown custom keys are preserved, and only paths explicitly retired by a named migration may be removed. Runtime-owned files under `data/` are never schema-pruned. Version 1.1.2 migrates the exact legacy reversed lobby-sidebar default into natural top-to-bottom order. Version 1.3.0 migrates only unchanged built-in command help and usage messages to the structured command paths. Version 1.4.0 similarly updates only unchanged kit-editor menu text and adds the Golden Head schema. Version 1.5.0 updates only unchanged mode-menu and match-result defaults to the compact Solo Duels presentation. Version 1.5.1 retires the old Select Mode / Select Map / Challenge Player hub. Version 1.6.0 adds the Solo/Team duel-type selector and all-mode personal kit editor. Version 1.6.1 renames the built-in sword to Queue Duels and adds the Other Modes category; customized active menu values remain untouched.

## Arena world lifecycle

Every `.slime` file in `World-Loader.Slime-World-Directory` is preloaded read-only by HyXDuels, including templates not yet referenced by a saved arena. Every nonblank world referenced by an arena's `Spawn-One`, `Spawn-Two`, or `Lobby` must be one of those templates. Both gameplay spawns of an arena must use the same world; that world is the arena's map template, while the lobby may use a different template. Runtime-created arenas can only use the preloaded read-only ASP instances. At startup, a same-named writable ASP or vanilla/Anvil world is unloaded without saving and replaced by the configured `.slime` template; Bukkit primary worlds that cannot be unloaded are rejected with an actionable configuration error. Slime loads deliberately suppress Bukkit `WorldLoadEvent`, preventing folder-oriented world managers from adopting them. An idle external unload used to remove a stale registry entry is allowed, admission-locked, and restored from `.slime` on the next tick. Bukkit autosave is disabled for every live arena wrapper, and arena location loading has no Bukkit fallback.
...
Every `/duels queue join` admission additionally requires each saved location to reference the exact current Bukkit `World` instance owned by ASP—not merely a world with the same name. This Bukkit object is ASP's required live interface for a slime world, not an Anvil-backed world created by HyXDuels. Immediately before teleport, HyXDuels synchronously probes the destination chunk and rechecks both the world identity and regeneration lock. A stale location or shut-down chunk system rejects and rolls back the admission without moving the player. Unexpected external unload requests for managed worlds are cancelled and rerouted through the same locked drain/regeneration lifecycle.
...
`/duels queue open` (`duels.gui`, granted by default) and the default glowing diamond sword named `Queue Duels` in hotbar slot 0 open `Play Duels`. That first menu contains `Solo Duels`, `Team Duels`, and `Other Modes`; each category then opens only its configured mode grid. The former Select Mode / Select Map / Challenge Player navigation hub remains retired. Selecting a single-kit mode such as Bed Wars immediately enters matchmaking; selecting a multi-kit mode such as Classic opens the kit menu first. Matchmaking keys include the canonical mode, kit, and combat profile, and `/duels queue leave` removes a waiting player from the queue. Two matching players are admitted atomically into a completely empty compatible arena.

The core arena runtime remains a strict two-participant 1v1 implementation. Team Duels validates a party leader with exactly two members and dispatches the console commands under `Menus.Duel-Type.Team.Commands`, with `<leader>`, `<party_size>`, `<mode>`, `<mode_key>`, `<kit>`, and `<kit_key>` placeholders. This connects the selector to the server's chosen four-player/team arena implementation without misrepresenting a two-player arena as native 2v2.

The shipped mode selector uses a compact configurable 17-slot grid. `Menus.Duel-Type.Solo.Mode-Keys`, `Team.Mode-Keys`, and `Other.Mode-Keys` control which modes appear in each category. Every tooltip includes a mode-specific description, the top three wins recorded today, and the live number of queued or admitted players. `Menus.Mode.Order`, `Content-Slots`, `Item-Name`, `Item-Lore`, and `Descriptions` can all be changed without code. The leaderboard values come from the same immutable cache as sidebars and holograms, so opening an inventory never performs a database query.

Arena sidebars have independent `Arena-Countdown` and `Arena-Playing` definitions. The countdown view shows map, occupancy, start time, mode, and plugin version; the playing view shows formatted time remaining, opponent health, score, and streak. Native display tokens also include `<time_formatted>`, both players' health/max-health values, `<players>`, `<max_players>`, and `<version>`. Final chat results show the winner, score, and remaining HP.
...
`advanced/menus.yml` independently configures menu titles, slots, materials, names, lore, party action commands, messages, settings entries, kit-editor controls, and tagged opener items. Openers support `DUEL_MENU`, `MODE_SELECTOR`, `MAP_SELECTOR`, `SETTINGS`, `PARTY`, and `KIT_EDITOR` plus material, slot, name, lore, glow, lock, force-slot, and custom-model-data settings. Items use a plugin PDC tag rather than their display name. Forced lobby slots temporarily preserve and later restore their original contents before duel admission, logout, or plugin shutdown; non-forced items fall back to the first empty inventory slot. Locked openers cannot be moved, swapped, or dropped. The shipped lobby hotbar places Queue Duels in slot 0, Create / Manage Party in slot 1, Kit Editor in slot 6, and Player Settings in slot 7. Creating a party replaces only the first two with Manage Party and Disband Party, prints the `/p` command reference, and requires confirmation before disbanding; Kit Editor and Player Settings remain unchanged.

The shared `Filler` section in `advanced/menus.yml` controls empty GUI slots across duel selectors, challenge kits, parties, settings, leaderboard filters, and the Bed Wars shop. It defaults to a gray stained-glass pane with `HyXDuels` lore and never replaces an interactive item. The kit editor fills only its non-editable control area so empty kit inventory, armor, and offhand slots remain editable.

`/duels help [page]` displays a configurable 3-10 entries per page (six by default), while `/duels kits` and `/duels arena` show short, focused subcommand summaries. Administrator arena operations use `/duels arena list|create|setlobby|setspawn1|setspawn2|finish|modes|settings|point|objective`. The former flat arena command names remain accepted as hidden compatibility forms but are no longer advertised or tab-completed.

`block-break` and `block-place` are administrator-owned per-arena settings, default false, and may be chosen while creating an arena with `/duels arena create <name> [block-break] [block-place]`. They remain editable under `Arenas.<id>.Settings` and `/duels arena settings <id> <flag> <true|false>`, and still require the selected mode's `Combat.block-damage` capability. Specialized Bed Wars and SkyWars construction remains available. Legacy 1.8 PvP is not an arena rule: it is the only player-selectable gameplay toggle in the mode menu. It removes modern attack cooldown/sweep behavior, is captured into queues and requests, and only matches equal preferences. Original attack speed and no-damage ticks are restored after every duel.

Active duel participants cannot recover health from hunger saturation, Peaceful difficulty, food healing, or custom passive healing. Only health potions/spells (`MAGIC`) and Regeneration effects (`MAGIC_REGEN`) are accepted. Existing version-one `modes.yml` files are migrated to `natural-regeneration: false`, and the strict mode parser rejects attempts to enable it again.
...
- `/kiteditor` (`duels.kits.layout`, granted by default) first shows every enabled mode, then any reusable kits for that mode, and edits that player's chosen kit layout. It persists by UUID and kit key in `data/kit-layouts.yml`, is applied when that player receives the kit, and never changes the shared kit. Personal saves must contain exactly the shared kit's item stacks rearranged; splitting, adding, removing, or changing items is rejected.
